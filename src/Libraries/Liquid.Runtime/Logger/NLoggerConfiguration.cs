...
namespace Liquid.Runtime
{
    ...
    public class NLoggerConfiguration : LightConfig<NLoggerConfiguration>
    {
        ...
        {
            RuleFor(d => FileName).NotEmpty().WithError("FileName on File settings should not be empty.");
            RuleFor(d => LogLevel).NotEmpty().WithError("LogLevel on File settings should not be empty."); 
